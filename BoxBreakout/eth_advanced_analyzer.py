...
class AdvancedBoxBreakoutAnalyzer:
    ...
    def zigzag_algorithm(self, highs: np.array, lows: np.array, deviation: float = 5.0, depth: int = 10):
        """
        ZigZag算法识别价格摆动点，使用OKX标准参数
        
        参数说明：
        - deviation: 最小价格变动百分比，默认5.0%（OKX标准）
        - depth: 寻找局部极值的深度，默认10（OKX标准）
        
        注：用户测试发现deviation=1效果很好，可根据需要调整
        """
        swing_points = []
        zigzag_line = []
    ...
    def identify_boxes_simple(self, prices: np.array, volumes: np.array, 
        ...
        调整参数以符合进阶版要求
        """
        swing_points, _ = self.zigzag_algorithm(highs, lows, deviation=3.5)
        boxes = []
    ...
    def identify_boxes_hybrid(self, prices: np.array, volumes: np.array, 
        ...
        # 获取ZigZag摆动点
        swing_points, zigzag_line = self.zigzag_algorithm(highs, lows, deviation=3.5)
...
def main():
    ...
    # 获取数据
    data = fetch_stock_data("sz.000063", start_date="2023-08-01", end_date="2025-09-30")
    ...
    # 创建分析器
    analyzer = AdvancedBoxBreakoutAnalyzer("sz.000063")
